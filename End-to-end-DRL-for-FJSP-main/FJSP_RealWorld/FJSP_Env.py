...
#--------------------------------------------------------------------------------------------------------------------
    ...
    def reset(self, data,rule=None):
        ...
        self.mchMat = -1 * np.ones((self.batch_sie,self.number_of_jobs,self.max_operation), dtype=np.int32)

        self.dur = data.astype(np.single)#single单精度浮点数
        self.dur_cp = deepcopy(self.dur)
        # record action history

        self.partial_sol_sequeence = -1 * np.ones((self.batch_sie,self.number_of_tasks),dtype=np.int32)
