"""
DE Solver HTTP Server — 远程调用接口，供 SkyEngine 通过 HTTP 调度
...
def _make_solver(config: dict) -> OnlineDESolver:
    return OnlineDESolver(**config)


def _serialize_result(result: dict, sv: OnlineDESolver) -> dict:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="DE Solver HTTP Server")
    parser.add_argument("--host", default="0.0.0.0")
    parser.add_argument("--port", type=int, default=8002)
    args = parser.parse_args()

    print(f"[DE Server] Starting on {args.host}:{args.port}")
