"""
PSO Solver HTTP Server — 远程调用接口，供 SkyEngine 通过 HTTP 调度
    ...
    for j in (raw_jobs if isinstance(raw_jobs, list) else []):
        ...
        for o in j.get("ops", []):
            ops.append(Op(
                ...
                proc_times=o.get("proc_times", {m: o.get("proc_time", 1.0) for m in o["machine_options"]}),
            ))
        jobs.append(Job(
            job_id=j["job_id"],
            ops=ops,
...
def _build_config(data: dict) -> dict:
    ...
    return {
        ...
        "w": cfg.get("w", 0.9),
        "lr": tuple(cfg.get("lr", [2, 2])),
        "seed": int(os.getenv("SEED", cfg.get("seed", 42))),
    }


def _make_solver(config: dict) -> OnlinePSOSolver:
    return OnlinePSOSolver(**config)


def _serialize_result(result: dict, sv: OnlinePSOSolver) -> dict:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="PSO Solver HTTP Server")
    parser.add_argument("--host", default="0.0.0.0")
    parser.add_argument("--port", type=int, default=8002)
    args = parser.parse_args()

    print(f"[PSO Server] Starting on {args.host}:{args.port}")
